		...
		for _, v := range contestantInfo.([]interface{}) {
			...
			if !isBiweek {
				...
				if ratingMap[tempContestant.Rank]["data"] == nil {
					tempContestant.AttendedContestCount = 0
					...
					if userContestRankingMap["userContestRanking"] != nil {
						...
						tempContestant.AttendedContestCount = (int)(lastMap["attendedContestsCount"].(float64))
						tempContestant.Rating = (lastMap["rating"].(float64))
					} else {
						tempContestant.AttendedContestCount = 0
						tempContestant.Rating = 1500.00
					}
...
func Test() {
	for _, v := range contestant {
		...
		for i := 0; i < p.NumField(); i++ {
			key := p.Type().Field(i).Name
			value := p.Field(i).Interface()
